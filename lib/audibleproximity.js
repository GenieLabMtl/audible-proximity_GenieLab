...
const DEFAULT_MAX_VOLUME_PING = 75;
...
class AudibleProximity {
  ...
  constructor(options) {
    ...
    this.audioFormat = options.audioFolderPath || DEFAULT_AUDIO_FORMAT;
    ...
    this.maxVolumePing = option.maxVolumePing || DEFAULT_MAX_VOLUME_PING;
    ...
    // always playing background
    //initPlayers(self, "data/audio/0000000.mp3", 5, Date.now() + this.staleThresholdMilliseconds * 60, "ac233fe64d7e/2");
...
function initPlayers(instance, file, targetVolume, timestamp, signature) {
  let audibleDevice = {
    file: file,
    targetVolume: targetVolume,
    timestamp: timestamp
  };
  instance.audibleDevices.set(signature, audibleDevice);
}
...
function handleRaddec(instance, raddec) {
  ...
  if(isKnownAudibleDevice) {
    ...
    if(isAudible) {
      ...
      initPlayers(instance, file, targetVolume, raddec.timestamp, raddec.signature);
    }
  }
}
  ...
  if(instance.audibleDevices.size === 0) {
    ...
    closestAudibleDevices.forEach((audibleDevice) => {
      if(concurrencyCount++ < instance.players.length) {
        let playerIndex = updatePlayers(instance, audibleDevice);
      ...
      for(let index = 0; index < availablePlayers.length; index++) {
        if(availablePlayers[index] === true) {
          ...
          if(audibleDevice.targetVolume > 0) {
            addFileToPlayer(instance, file, index);
          }
          else {
 ...
 */
function updatePlayers(instance, audibleDevice) {
  let playerIndex = null;

  instance.players.forEach((player, index) => {
    if(player.file === audibleDevice.file) {
      ...
      if(player.instance.track === null) {
        addFileToPlayer(instance, audibleDevice.file, playerIndex);
      }
    }
  });
...
function addFileToPlayer(instance, file, index) {
  if (player.volume < instance.maxVolumePing) {
    let pingNumber = Math.floor(volume/25)*30 + 80;
    file = instance.audioFolderPath + pingNumber + instance.audioFormat;
  }
  instance.players[index].instance.volume.play(file);
}
