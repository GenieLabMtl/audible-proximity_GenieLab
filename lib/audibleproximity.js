 ...
 */
 ...
 function createPlayers(options) {
   ...
   for(let cPlayer = 0; cPlayer < maxConcurrentPlayers; cPlayer++) {
     let player = { instance: new mpg.MpgPlayer(), file: null, fileId: null, volume: 0 };
     ...
     for(let index = 0; index < availablePlayers.length; index++) {
       if(availablePlayers[index] === true) {
         ...
         if (player.volume > 50 ) {
          ...
          player.file = './data/audio/0000008.wav';
         }
         player.fileId = audibleDevice.file;
 ...
 function updatePlayers(players, audibleDevice) {
   ...
   players.forEach((player, index) => {
     if(player.fileId === audibleDevice.file) {
       ...
       if(player.instance.track === null) {
         if (player.volume > 50 ) {
          ...
          player.instance.play('./data/audio/0000008.wav');
         }
       }
     }
   });
